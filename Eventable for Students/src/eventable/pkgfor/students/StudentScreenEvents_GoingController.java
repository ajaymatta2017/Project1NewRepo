...
public class StudentScreenEvents_GoingController extends Application implements Initializable {
    ...
    public void populateTableView() throws SQLException {
        ...
        currentQuery = "SELECT EVENT_TITLE, CAST(TO_CHAR(EVENT_START, 'dd/MON/yy') AS VARCHAR2(50)), LOCATION_TYPE, STREET_NO, STREET_NAME, POSTCODE, SUBURB, BUILDING_CODE, BUILDING_NAME, ROOM_NO, SOCIETY_NAME, CAST(TO_CHAR(EVENT_END, 'dd/MON/yy') AS VARCHAR2(50)), CAST(TO_CHAR(EVENT_END, 'hh:mm') AS VARCHAR2(50)), CAST(TO_CHAR(EVENT_START, 'hh:mm') AS VARCHAR2(50)), event_text, event_id FROM EVENT e JOIN SOCIETY USING(SOCIETY_ID) LEFT OUTER JOIN CAMPUS USING(ROOM_NO, BUILDING_CODE) JOIN ATTENDANCE a USING (event_id) WHERE a.event_theoretical_attendance = 'Y' and e.EVENT_START >= '05/MAY/2018' and a.email = '" + loggedInUser + "'";
